  ...
  <nav class="navbar navbar-default" role="navigation">
    ...
    <div class="collapse navbar-collapse navbar-ex1-collapse">
      <ul class="nav navbar-nav">
        <li v-bind:class="[$route.name==='explorer' ? 'active' : '']"><a v-link="{name: 'explorer'}">Explorar</a></li> 
...
<script>
  ...
  methods: {
    showStadist:function(){
      this.zoomModal=true;
      this.$http.get('http://ngexplorer-beta.prod.uci.cu/user/counter', function(res) {
        this.stadist=res;
    ...
    counterVisit:function(data){
     this.$http.post('http://ngexplorer-beta.prod.uci.cu/user/counter', data, function(res) {
      this.counter=res;
    });
